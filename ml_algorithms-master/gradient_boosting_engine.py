...
class LossFunctionMinimizer(nn.Module):
    ...
    def loss(self, previous_predictions, targets_leaf_tensor):
        ...
        elif self.type == "multiclassifier":
            return self.loss_multiclassifier(previous_predictions, targets_leaf_tensor)
        else:
    ...
    def loss_regressor(self, previous_predictions, targets_leaf_tensor):
        ...
        return loss        
    def loss_multiclassifier(self, previous_predictions, targets_leaf_tensor):
        logodds = self.refine_previous_predictions(previous_predictions)
        results = torch.zeros_like(logodds)
        for i in range(logodds.shape[1]):
            x = logodds[:,i] - torch.max(logodds[:,i])
            expo = torch.exp(x)
            expo_sum = torch.sum(torch.exp(x))
            result[:,i] = torch.exp(x)/torch.sum(np.exp(x))
        loss = F.cross_entropy(probabilities, targets_leaf_tensor)
        return loss
#######################################################################################################
'''
...
class PytorchBasedGenericGradientBoost():
    ...
    def predict(self, X):
        ...
        if self.type == "regressor":
            ...
            return torch.softmax(torch.tensor(predictions)).numpy()
        else:
            raise Exception("Not supported")